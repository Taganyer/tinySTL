...
namespace STD {
    ...
    class Red_Black_Tree {
    public:
        friend class Detail::Red_Black_Tree_Iterator<Key, Compare, Equal_>;

        using Iterator = Detail::Red_Black_Tree_Iterator<Key, Compare, Equal_>;
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    template<typename Key, typename Compare, typename Equal_>
    Pair<typename Red_Black_Tree<Key, Compare, Equal_>::Iterator, bool>
    Red_Black_Tree<Key, Compare, Equal_>::insert(Key &&key) {
        Size record = size_;
        //插入节点为红节点，可以强迫结构的变换。
        Node *temp = Allocate<Node>(move(key)), *judge = nullptr;
        root = insert(root, temp, judge);
        root->Red = false;
        root->last = nullptr;
        if (judge) {
