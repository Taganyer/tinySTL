...
template <typename Arg> class Deque {
  private:
    Size size_ = 0;
  ...
  public:
    ...
    template <typename... args> void emplace_front(args &&...vals);
    ...
    template <typename... args> void emplace_back(args &&...vals);
    ...
    template <typename... args> Iterator emplace(Size pos, args &&...vals);
    ...
    Iterator insert(const Iterator &pos, const Iter<Arg> &begin,
                    const Iter<Arg> &end);

    Iterator insert(const Iterator &pos, const cIter<Arg> &begin,
                    const cIter<Arg> &end);

    cIterator insert(const cIterator &pos, Size size, const Arg &val);

    cIterator insert(const cIterator &pos,
                     const std::initializer_list<Arg> &list);

    cIterator insert(const cIterator &pos, const Iter<Arg> &begin,
                     const Iter<Arg> &end);

    cIterator insert(const cIterator &pos, const cIter<Arg> &begin,
                     const cIter<Arg> &end);
    ...
    rIterator insert(const rIterator &pos, const Iter<Arg> &begin,
                     const Iter<Arg> &end);
    ...
    Arg &at(Size pos) const {
        if (pos >= size_)
            throw outOfRange("You provided an out-of-range subscript int the "
                             "'Deque::at' function");
        auto temp = find_ptr(pos);
        return temp.first[0][temp.second];
    };
    ...
    Arg &front() const {
        if (!size_)
            throw outOfRange("You're accessing a non-existent element in the "
                             "'Deque::front' function");
    ...
    Iterator begin() const {
        return Deque<Arg>::Iterator(*(map + map_begin) + val_begin,
                                    map + map_begin, val_begin);
    };

    Iterator end() const {
        return Deque<Arg>::Iterator(*(map + map_end) + val_end, map + map_end,
                                    val_end);
    };

    cIterator cbegin() const {
        return Deque<Arg>::cIterator(*(map + map_begin) + val_begin,
                                     map + map_begin, val_begin);
    };

    cIterator cend() const {
        return Deque<Arg>::cIterator(*(map + map_end) + val_end, map + map_end,
                                     val_end);
    };

    rIterator rbegin() const {
        auto temp = find_ptr(size_ - 1);
        return Deque<Arg>::rIterator(*temp.first + temp.second, temp.first,
                                     temp.second);
    };
    ...
    crIterator crbegin() const {
        auto temp = find_ptr(size_ - 1);
        return Deque<Arg>::crIterator(*temp.first + temp.second, temp.first,
                                      temp.second);
    };
...
template <typename Arg> void Deque<Arg>::init() {
...
template <typename Arg> Deque<Arg>::Deque() { init(); }
...
template <typename Arg> Deque<Arg>::~Deque<Arg>() {
    for (int i = 0; i < map_size; ++i)
        Deallocate_n(map[i]);
    Deallocate_n(map);
}

template <typename Arg> void Deque<Arg>::clear() {
    size_ = 0;
    for (int i = 0; i < map_size; ++i)
        Deallocate_n(map[i]);
    Deallocate_n(map);
...
template <typename Arg> void Deque<Arg>::shirk_to_fit() {
...
template <typename Arg> void Deque<Arg>::push_front(const Arg &val) {
    if (map_begin == 0 && val_begin == 0)
        expand_capacity(1, false);
    subtract(map_begin, val_begin);
...
template <typename Arg> void Deque<Arg>::push_front(Arg &&val) {
    if (map_begin == 0 && val_begin == 0)
        expand_capacity(1, false);
    subtract(map_begin, val_begin);
...
template <typename Arg> void Deque<Arg>::push_front(Size size, const Arg &val) {
    auto rest = left_rest();
    if (rest < size)
...
template <typename Arg> void Deque<Arg>::pop_front() {
...
template <typename Arg> void Deque<Arg>::push_back(const Arg &val) {
    if (map_end == map_size)
        expand_capacity(1, true);
...
template <typename Arg> void Deque<Arg>::push_back(Arg &&val) {
    if (map_end == map_size)
        expand_capacity(1, true);
...
template <typename Arg> void Deque<Arg>::push_back(Size size, const Arg &val) {
...
template <typename Arg> void Deque<Arg>::pop_back() {
...
template <typename Arg>
template <typename... args>
typename Deque<Arg>::Iterator Deque<Arg>::emplace(const Deque::Iterator &pos,
                                                  args &&...vals) {
...
template <typename Arg>
template <typename... args>
typename Deque<Arg>::cIterator Deque<Arg>::emplace(const Deque::cIterator &pos,
                                                   args &&...vals) {
...
template <typename Arg>
template <typename... args>
typename Deque<Arg>::rIterator Deque<Arg>::emplace(const Deque::rIterator &pos,
                                                   args &&...vals) {
...
Deque<Arg>::emplace(const Deque::crIterator &pos, args &&...vals) {
...
template <typename Arg>
typename Deque<Arg>::Iterator Deque<Arg>::insert(Size pos, const Arg &val,
                                                 Size size) {
    if (pos > size_)
        throw outOfRange(
...
template <typename Arg>
typename Deque<Arg>::Iterator Deque<Arg>::insert(const Deque::Iterator &pos,
                                                 Size size, const Arg &val) {
    return insert(find_pos(pos.map, pos.pos), size, val);
}

template <typename Arg>
typename Deque<Arg>::Iterator
Deque<Arg>::insert(const Deque::Iterator &pos,
                   const std::initializer_list<Arg> &list) {
...
template <typename Arg>
typename Deque<Arg>::Iterator Deque<Arg>::insert(const Deque::Iterator &pos,
                                                 const Iter<Arg> &begin,
                                                 const Iter<Arg> &end) {
    return insert(find_pos(pos.map, pos.pos), *begin.to_const(),
                  *end.to_const());
}

template <typename Arg>
typename Deque<Arg>::Iterator Deque<Arg>::insert(const Deque::Iterator &pos,
                                                 const cIter<Arg> &begin,
                                                 const cIter<Arg> &end) {
    return insert(find_pos(pos.map, pos.pos), begin, end);
}

template <typename Arg>
typename Deque<Arg>::cIterator Deque<Arg>::insert(const Deque::cIterator &pos,
                                                  Size size, const Arg &val) {
...
Deque<Arg>::insert(const Deque::cIterator &pos,
                   const std::initializer_list<Arg> &list) {
...
typename Deque<Arg>::cIterator Deque<Arg>::insert(const Deque::cIterator &pos,
                                                  ...
                                                  const Iter<Arg> &end) {
    Iterator temp =
        insert(find_pos(pos.map, pos.pos), *begin.to_const(), *end.to_const());
    return Deque::cIterator(temp.target, temp.map, temp.pos);
}
...
template <typename Arg>
typename Deque<Arg>::rIterator Deque<Arg>::insert(const Deque::rIterator &pos,
                                                  Size size, const Arg &val) {
    auto index = find_pos(pos.map, pos.pos) + 1;
    if (index > size_)
        throw outOfRange(
...
Deque<Arg>::insert(const Deque::rIterator &pos,
                   const std::initializer_list<Arg> &list) {
    auto size = list.size();
    auto index = find_pos(pos.map, pos.pos) + 1;
    if (index > size_)
...
template <typename Arg>
typename Deque<Arg>::crIterator Deque<Arg>::insert(const Deque::crIterator &pos,
                                                   Size size, const Arg &val) {
...
template <typename Arg>
typename Deque<Arg>::crIterator
Deque<Arg>::insert(const Deque::crIterator &pos,
                   const std::initializer_list<Arg> &list) {
...
typename Deque<Arg>::crIterator Deque<Arg>::insert(const Deque::crIterator &pos,
                                                   const Iter<Arg> &begin,
                                                   const Iter<Arg> &end) {
    rIterator temp = insert(rIterator(pos.target, pos.map, pos.pos),
                            *begin.to_const(), *end.to_const());
    return Deque::crIterator(temp.target, temp.map, temp.pos);
}
...
typename Deque<Arg>::crIterator Deque<Arg>::insert(const Deque::crIterator &pos,
                                                   ...
                                                   const cIter<Arg> &end) {
    rIterator temp =
        insert(rIterator(pos.target, pos.map, pos.pos), begin, end);
    return Deque::crIterator(temp.target, temp.map, temp.pos);
}
...
typename Deque<Arg>::Iterator Deque<Arg>::erase(Size pos, Size size) {
    if (pos > size_)
        throw outOfRange(
            "You passed an out-of-range value in the 'Deque::erase' function");
    auto temp = find_ptr(pos);
    if (size == 0)
        return Deque<Arg>::Iterator(*temp.first + temp.second, temp.first,
                                    temp.second);
...
template <typename Arg>
typename Deque<Arg>::Iterator Deque<Arg>::erase(const Deque::Iterator &begin,
                                                const Deque::Iterator &end) {
    return erase(find_pos(begin.map, begin.pos), calculateLength(begin, end));
}

template <typename Arg>
typename Deque<Arg>::cIterator Deque<Arg>::erase(const Deque::cIterator &begin,
                                                 const Deque::cIterator &end) {
    Iterator temp =
        erase(find_pos(begin.map, begin.pos), calculateLength(begin, end));
    return Deque::cIterator(temp.target, temp.map, temp.pos);
}
...
template <typename Arg>
typename Deque<Arg>::crIterator
Deque<Arg>::erase(const Deque::crIterator &iter) {
    Iterator temp = erase(find_pos(iter.map, iter.pos), 1);
    return Deque::crIterator(temp.target, temp.map, temp.pos);
}

template <typename Arg>
typename Deque<Arg>::rIterator Deque<Arg>::erase(const Deque::rIterator &begin,
                                                 const Deque::rIterator &end) {
    Iterator temp =
        erase(find_pos(end.map, end.pos) + 1, calculateLength(begin, end));
    return Deque::rIterator(temp.target, temp.map, temp.pos);
}

template <typename Arg>
typename Deque<Arg>::crIterator
Deque<Arg>::erase(const Deque::crIterator &begin,
                  const Deque::crIterator &end) {
    Iterator temp =
        erase(find_pos(end.map, end.pos) + 1, calculateLength(begin, end));
    return Deque::crIterator(temp.target, temp.map, temp.pos);
}
...
template <typename Arg> class Deque<Arg>::Iterator : public Random_Iter<Arg> {
  protected:
  ...
  public:
    ...
    Shared_ptr<Iter<Arg>> deep_copy() const override {
        return make_shared<Deque<Arg>::Iterator>(*this);
    };

    Shared_ptr<cIter<Arg>> to_const() const override {
        return make_shared<Deque<Arg>::cIterator>(
            Deque<Arg>::cIterator(target, map, pos));
    };
...
template <typename Arg> class Deque<Arg>::cIterator : public cRandom_Iter<Arg> {
  protected:
  ...
  public:
    ...
    Shared_ptr<cIter<Arg>> deep_copy() const override {
        return make_shared<Deque<Arg>::cIterator>(*this);
    };
...
template <typename Arg> class Deque<Arg>::rIterator : public Random_Iter<Arg> {
  protected:
    using Iter<Arg>::target;
  ...
  public:
    ...
    Shared_ptr<Iter<Arg>> deep_copy() const override {
        return make_shared<Deque<Arg>::rIterator>(*this);
    };

    Shared_ptr<cIter<Arg>> to_const() const override {
        return make_shared<Deque<Arg>::crIterator>(
            Deque<Arg>::crIterator(target, map, pos));
    };
...
class Deque<Arg>::crIterator : public cRandom_Iter<Arg> {
  ...
  public:
    ...
    Shared_ptr<cIter<Arg>> deep_copy() const override {
        return make_shared<Deque<Arg>::crIterator>(*this);
    };
